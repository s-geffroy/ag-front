...
- `presentations/` — **client-facing decks, versioned** (ADR 0073). Generated by
  `scripts/build-deck.sh` from `packages/deck` (`@ag/deck`), never written by hand: the prices and the
  positioning come from `apps/public/src/lib/site.ts`, so the plaquette cannot diverge from `/offres`.
  Conversion + QA run in the **`slides`** image (LibreOffice, poppler, qpdf, the `pptx`/`pdf` skills),
  built on demand — `docker compose -f docker/docker-compose.yml build slides`. Publishing is a
  **cockpit** act at `/commercial/plaquette`, gated on `manifest.json#published`; until then the
  `/plaquette` page is withheld from the served build. Pass `--date` for a byte-reproducible rebuild.
