...
- `presentations/` — **client-facing decks, versioned** (ADR 0073). Two families, published
  independently: `commercial` (16 slides, cold prospecting) and `methode` (29 slides, the backend +
  CVI/HDDE/VERDICT + a worked walkthrough). Generated by `scripts/build-deck.sh` from `packages/deck`
  (`@ag/deck`), never written by hand: prices come from `apps/public/src/lib/site.ts`, method
  vocabulary from `@ag/cvi` / `@ag/verdict`, backend figures from `src/backend-facts.ts` (each with its
  counting method). **A price change on `/offres` requires regenerating `commercial`** — nothing
  reminds you. Conversion + QA run in the **`slides`** image (LibreOffice, poppler, qpdf, the
