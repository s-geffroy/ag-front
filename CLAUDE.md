...
- `presentations/` — **client-facing decks, versioned** (ADR 0073). Two families, published
  independently: `commercial` (cold prospecting) and `methode` (the backend + CVI/HDDE/VERDICT + a
  worked walkthrough). The slide count per family and language lives in
  `presentations/<family>/manifest.json` and is written by the build — 15 and 29 at 2026-08-21; this
  file does not repeat it, because it said 16 for six weeks after a slide was removed. Generated by
  `scripts/build-deck.sh` from `packages/deck` (`@ag/deck`), never written by hand: prices come from
  `apps/public/src/lib/site.ts`, method vocabulary from `@ag/cvi` / `@ag/verdict`, backend figures
  from `src/backend-facts.ts` (each with its counting method). **A price change on `/offres` requires
  regenerating `commercial`, and so does any change to `backend-facts.ts` / `cvi-facts.ts`** — nothing
  reminds you, and a figure corrected in the code beside a stale one in a versioned PDF is the worst
  of the two states. Conversion + QA run in the **`slides`** image (LibreOffice, poppler, qpdf, the
  `pptx`/`pdf` skills), built on demand — `docker compose -f docker/docker-compose.yml build slides`.
  Publishing is a **cockpit** act at `/commercial/plaquette`, per family, gated on
  `manifest.json#published`; while none is published the `/plaquette` page is withheld from the served
  build. Pass `--date` for a byte-reproducible rebuild.
...
| Build or QA a **`.pptx` deck** — the `presentations/` plaquettes                                  | `pptx` (scripts run in the **`slides`** container, ADR 0073)                               |
| Inspect / normalise / extract from a **`.pdf`**                                                   | `pdf` (scripts run in the **`slides`** container, ADR 0073)                                |
