from http import HTTPStatus
from typing import Any, Optional, Union, cast

import httpx

from ...client import AuthenticatedClient, Client
from ...types import Response, UNSET
from ... import errors

from ...models.http_validation_error import HTTPValidationError
from typing import cast



def _get_kwargs(
    chokepoint_id: str,
    doc: str,

) -> dict[str, Any]:
    

    

    

    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/chokepoint-analyses/{chokepoint_id}/{doc}".format(chokepoint_id=chokepoint_id,doc=doc,),
    }


    return _kwargs


def _parse_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Optional[Union[HTTPValidationError, str]]:
    if response.status_code == 200:
        response_200 = response.text
        return response_200
    if response.status_code == 422:
        response_422 = HTTPValidationError.from_dict(response.json())



        return response_422
    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Response[Union[HTTPValidationError, str]]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    chokepoint_id: str,
    doc: str,
    *,
    client: AuthenticatedClient,

) -> Response[Union[HTTPValidationError, str]]:
    """ Get Chokepoint Analysis Doc

     Raw Markdown of one analysis doc (synthesis | theory-of-constraints | leverage-points).

    Args:
        chokepoint_id (str):
        doc (str):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Union[HTTPValidationError, str]]
     """


    kwargs = _get_kwargs(
        chokepoint_id=chokepoint_id,
doc=doc,

    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)

def sync(
    chokepoint_id: str,
    doc: str,
    *,
    client: AuthenticatedClient,

) -> Optional[Union[HTTPValidationError, str]]:
    """ Get Chokepoint Analysis Doc

     Raw Markdown of one analysis doc (synthesis | theory-of-constraints | leverage-points).

    Args:
        chokepoint_id (str):
        doc (str):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Union[HTTPValidationError, str]
     """


    return sync_detailed(
        chokepoint_id=chokepoint_id,
doc=doc,
client=client,

    ).parsed

async def asyncio_detailed(
    chokepoint_id: str,
    doc: str,
    *,
    client: AuthenticatedClient,

) -> Response[Union[HTTPValidationError, str]]:
    """ Get Chokepoint Analysis Doc

     Raw Markdown of one analysis doc (synthesis | theory-of-constraints | leverage-points).

    Args:
        chokepoint_id (str):
        doc (str):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Union[HTTPValidationError, str]]
     """


    kwargs = _get_kwargs(
        chokepoint_id=chokepoint_id,
doc=doc,

    )

    response = await client.get_async_httpx_client().request(
        **kwargs
    )

    return _build_response(client=client, response=response)

async def asyncio(
    chokepoint_id: str,
    doc: str,
    *,
    client: AuthenticatedClient,

) -> Optional[Union[HTTPValidationError, str]]:
    """ Get Chokepoint Analysis Doc

     Raw Markdown of one analysis doc (synthesis | theory-of-constraints | leverage-points).

    Args:
        chokepoint_id (str):
        doc (str):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Union[HTTPValidationError, str]
     """


    return (await asyncio_detailed(
        chokepoint_id=chokepoint_id,
doc=doc,
client=client,

    )).parsed
