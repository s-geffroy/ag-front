...
const PACKET = {
  ...
  corridor_cvi: {
    ...
    uncertainties: [],
    dimensions: {
      menace: { score: 4, rationale: 'Acteurs étatiques hostiles au débouché', confidence: 'moyen' },
      gouvernance: { score: 3, rationale: 'Gouvernance du corridor fragmentée', confidence: 'bas' },
    },
  },
  concentration: {
    supplier_count: 5, customer_count: 3, site_count: 2, single_source_supplier_count: 1,
    tier2_blind_spots: 2, customer_top_share_pct: 40, customer_hhi: 0.3,
    supplier_top_country: 'CN', supplier_top_country_count: 3, notes: [],
  },
};
...
beforeAll(async () => {
  ...
  globalThis.fetch = (async (input: any, init?: any) => {
    ...
    if (typeof url === 'string' && url.includes('hdde-test')) {
      ...
      const body = url.includes('draft-case') ? { ...INTERNAL_BODY, status: 'draft' } : INTERNAL_BODY;
...
describe('VERDICT ingestion from an HDDE packet', () => {
  ...
  it('logs in and creates a decision', async () => {
    const login = await api('POST', '/api/auth/login', { email: 'analyst@example.com', password: 'correct-horse-battery' });
  ...
  it('ingests the HDDE packet and pre-fills candidates', async () => {
    const res = await api('POST', `/api/decisions/${decisionId}/ingest`, { hdde_case_ref: 'case-1' });
  ...
  it('exposes candidates with provenance and records the source on the decision', async () => {
    ...
    expect(bundle.swot.every((s: { status: string; source_kind: string }) => s.status === 'candidate' && s.source_kind !== 'manual')).toBe(true);
    expect(bundle.swot.some((s: { source_ref: string }) => s.source_ref === 'hdde:red_flag:tier2_unknown')).toBe(true);
    ...
    expect(bundle.pestel.some((p: { source_kind: string; source_ref: string }) => p.source_kind === 'chokepoint' && p.source_ref === 'chokepoint:cp_ormuz')).toBe(true);
    expect(bundle.swot.some((s: { source_kind: string; source_ref: string }) => s.source_kind === 'chokepoint' && s.source_ref === 'chokepoint:cp_ormuz')).toBe(true);
    // Per-corridor CVI assessment → SWOT Threat (cvi:menace) + PESTEL Legal (cvi:gouvernance).
    expect(bundle.swot.some((s: { source_kind: string; source_ref: string }) => s.source_kind === 'cvi' && s.source_ref === 'cvi:menace')).toBe(true);
    expect(bundle.pestel.some((p: { source_kind: string; source_ref: string }) => p.source_kind === 'cvi' && p.source_ref === 'cvi:gouvernance')).toBe(true);
  });

  it('refuses a non-validated (draft) packet — ingest 502 (doctrine: human validation mandatory)', async () => {
    const res = await api('POST', `/api/decisions/${decisionId}/ingest`, { hdde_case_ref: 'draft-case' });
