...
const wrap = (fn: AsyncHandler) => (req: CaseRequest, res: Response, next: NextFunction) =>
...
casesRouter.post(
  ...
  wrap(async (req: CaseRequest, res) => {
    ...
    if (
      ...
      res
        .status(429)
        .json({ error: 'llm_budget_exceeded', detail: 'Plafond LLM quotidien atteint.' });
      return;
    }
