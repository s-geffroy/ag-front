      ...
      <header className="border-b border-slate-300 bg-white dark:border-slate-700 dark:bg-slate-800">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
          <Link to="/" className="font-semibold tracking-tight">
            VERDICT{' '}
            <span className="font-normal text-slatewarn dark:text-slate-400">· {fr.app.title}</span>
          </Link>
...
export default function App() {
  ...
  return (
    <Shell>
      <Routes>
        <Route path="/" element={<DecisionsList />} />
        <Route path="/decisions/:id" element={<DecisionWorkspace />} />
