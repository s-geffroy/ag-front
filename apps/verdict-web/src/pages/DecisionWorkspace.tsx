...
const OPTION_TYPE_FR: Record<string, string> = {
  ...
  opposite: 'Opposée',
  active_non_action: 'Non-action active',
};
const CANVAS_FIELDS = [
...
const auditClass: Record<string, string> = {
  VALIDE: 'audit-VALIDE',
  'À CORRIGER': 'audit-CORRIGER',
  BLOQUÉ: 'audit-BLOQUE',
};
    ...
    <div>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {d.final_verdict && (
            <span className={`chip v-${asClass(d.final_verdict)}`}>{d.final_verdict}</span>
          )}
          {audit && (
      ...
      <p className="text-xs text-slatewarn dark:text-slate-400">
        ...
        {d.source_pack_hash && (
          <>
            {' '}
            · ingéré de HDDE (<code>{d.hdde_case_ref}</code>)
          </>
        )}
      </p>
      ...
      <div className="mt-4">
        ...
        {step === 'decide' && (
          <DecideStage id={id} d={d} options={bundle.options} audit={audit} onDone={invalidate} />
        )}
      </div>
    </div>
  );
...
function normalizeAudit(
  row: Row | null,
): { audit_status: string; blocking_errors: string[]; warnings: string[] } | null {
  if (!row) return null;
  if (row.result_json) {
    try {
...
function CandidateActions({
  ...
  const setStatus = useMutation({
    mutationFn: (status: string) =>
      api.patch(`/api/decisions/${id}/${kind}/${item.id}`, { status }),
    onSuccess: onDone,
  });
  ...
  return (
    <div className="mt-1 flex gap-2 text-xs">
      {item.status !== 'validated' && (
        <button
          onClick={() => setStatus.mutate('validated')}
      ...
      <button onClick={() => del.mutate()} className="text-red-600 hover:underline">
        {fr.ws.remove}
      </button>
    </div>
  );
...
function Ingest({ id, onDone }: { id: string; onDone: () => void }) {
  ...
  const ingest = useMutation({
    mutationFn: () =>
      api.post<{ ingested: { pestel: number; swot: number; options: number } }>(
        `/api/decisions/${id}/ingest`,
        { hdde_case_ref: ref },
      ),
    onSuccess: (r) => {
    ...
    onError: (e) =>
      setMsg(e instanceof ApiError && e.status === 502 ? fr.ws.ingestFail : String(e)),
  });
  return (
  ...
  return (
    <section className="card">
      ...
      <p className="mt-1 text-sm text-slatewarn dark:text-slate-400">
        Énoncez la situation sans solution préférée.
      </p>
      <textarea
        value={situation}
      ...
      />
      <button className="btn mt-3" disabled={save.isPending} onClick={() => save.mutate()}>
        {fr.ws.save}
      </button>
    </section>
  );
...
function PestelStage({ id, items, onDone }: { id: string; items: Row[]; onDone: () => void }) {
  ...
  return (
    <section className="card">
      ...
      <p className="mt-1 text-sm text-slatewarn dark:text-slate-400">
        Uniquement les facteurs qui changent le coût, le risque, le timing ou une hypothèse d’une
        option.
      </p>
            ...
            <ul className="mt-2 space-y-2">
              {items
                ...
                .map((f) => (
                  <li
                    key={f.id}
                    className={`rounded border border-slate-200 bg-white p-2 text-sm dark:border-slate-700 dark:bg-slate-900 ${f.status === 'rejected' ? 'opacity-50' : ''}`}
                  >
                    <div>{f.statement}</div>
                    {f.decisional_impact && (
        ...
        fields={[
          {
            ...
            label: 'Catégorie',
            type: 'select',
            options: PESTEL_CATEGORIES.map((c) => [c, PESTEL_FR[c]]),
          },
...
function SwotStage({ id, items, onDone }: { id: string; items: Row[]; onDone: () => void }) {
  ...
  return (
    <section className="card">
      ...
      <p className="mt-1 text-sm text-slatewarn dark:text-slate-400">
        Une force sans preuve est une hypothèse.
      </p>
            ...
            <ul className="mt-2 space-y-2">
              {items
                ...
                .map((s) => (
                  <li
                    key={s.id}
                    className={`rounded border border-slate-200 bg-white p-2 text-sm dark:border-slate-700 dark:bg-slate-900 ${s.status === 'rejected' ? 'opacity-50' : ''}`}
                  >
                    <div>
                      {s.statement}
                      {s.is_hypothesis ? (
                        <em className="text-xs text-slatewarn"> (hypothèse)</em>
                      ) : null}
                    </div>
                    <div className="mt-1">
                      <Provenance row={s} />
        ...
        fields={[
          {
            ...
            label: 'Quadrant',
            type: 'select',
            options: SWOT_QUADRANTS.map((q) => [q, SWOT_FR[q]]),
          },
...
function OptionsStage({ id, options, onDone }: { id: string; options: Row[]; onDone: () => void }) {
  ...
  const missing = OPTION_TYPES.filter((t) =>
    t === 'opposite' || t === 'active_non_action' ? false : !present.has(t),
  );
        ...
        <p className="mt-1 text-sm text-slatewarn dark:text-slate-400">
          ...
          {(missing.length > 0 || needOpposite) && (
            <span className="ml-1 text-amber-600">
              Manque :{' '}
              {[...missing.map((m) => OPTION_TYPE_FR[m]), needOpposite ? 'Opposée/Non-action' : '']
                .filter(Boolean)
                .join(', ')}
              .
            </span>
    ...
    <div className="card">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold">
          {isNew
            ? 'Nouvelle option'
            : `${OPTION_TYPE_FR[form.type]} — ${form.title || form.option_id}`}
        </h3>
      ...
      <div className="mt-2 grid gap-2 sm:grid-cols-2">
        <Labelled label="Identifiant">
          <input
            ...
            disabled={!isNew}
            onChange={(e) => set('option_id', e.target.value)}
          />
        </Labelled>
        <Labelled label="Type">
        ...
        <Labelled label="Hypothèse critique" full>
          <input
            className="field"
            value={form.critical_hypothesis}
            onChange={(e) => set('critical_hypothesis', e.target.value)}
          />
        </Labelled>
        <Labelled label="Preuve principale">
          <input
            className="field"
            value={form.main_evidence}
            onChange={(e) => set('main_evidence', e.target.value)}
          />
        </Labelled>
        <Labelled label="Contradiction principale">
          <input
            className="field"
            value={form.main_contradiction}
            onChange={(e) => set('main_contradiction', e.target.value)}
          />
        </Labelled>
        <Labelled label={`Niveau de preuve : ${form.proof_level}`} full>
          <input
            ...
            min={0}
            max={5}
            value={form.proof_level}
            onChange={(e) => set('proof_level', Number(e.target.value))}
            className="w-full"
          />
          <span className="text-xs text-slatewarn dark:text-slate-400">
            {proofLevels[form.proof_level]?.label}
          </span>
        </Labelled>
      </div>
      <fieldset className="mt-3 rounded-lg border border-slate-200 p-3 dark:border-slate-700">
        <legend className="px-1 text-xs font-semibold text-slatewarn dark:text-slate-400">
          ...
          {CANVAS_FIELDS.map(([k, label]) => (
            <Labelled key={k} label={label}>
              <input
                className="field"
                value={form.canvas[k] ?? ''}
                onChange={(e) => set('canvas', { ...form.canvas, [k]: e.target.value })}
              />
            </Labelled>
          ))}
        </div>
      ...
      <div className="mt-3 flex gap-2">
        <button
          className="btn"
          disabled={!form.option_id || !form.title || save.isPending}
          onClick={() => save.mutate()}
        >
          {fr.ws.save}
        </button>
        {!isNew && (
          <button className="btn-ghost text-red-600" onClick={() => del.mutate()}>
            {fr.ws.remove}
          </button>
        )}
      </div>
...
function InterrogateStage({
  ...
  const run = useMutation({
    mutationFn: () =>
      api.post(`/api/decisions/${id}/red-team/run`, { role, target_option_id: target || null }),
    onSuccess: () => {
      setErr('');
      onDone();
    },
    onError: (e) =>
      setErr(e instanceof ApiError && e.status === 502 ? 'Échec de l’appel LLM.' : String(e)),
  });
  const review = useMutation({
    mutationFn: ({ sid, status }: { sid: string; status: string }) =>
      api.patch(`/api/decisions/${id}/red-team/${sid}`, { status }),
    onSuccess: onDone,
  });
  return (
    <section className="space-y-3">
      <div className="card">
        ...
        <p className="mt-1 text-sm text-slatewarn dark:text-slate-400">
          Pour chaque option : hypothèse, preuve, contradiction, niveau de preuve.
        </p>
      ...
      <div className="card">
        ...
        <p className="mt-1 text-xs text-slatewarn dark:text-slate-400">
          Sortie = suggestion (preuve niveau 0), jamais un verdict. L’analyste accepte ou rejette.
        </p>
        <div className="mt-2 flex flex-wrap items-end gap-2">
          <label className="text-sm">
            <span className="font-medium">Mode</span>
            <select className="field" value={role} onChange={(e) => setRole(e.target.value)}>
              {RED_TEAM_ROLES.map(([v, l]) => (
        ...
        <ul className="mt-3 space-y-2">
          {redTeam.length === 0 && (
            <li className="text-sm text-slatewarn dark:text-slate-400">Aucune suggestion.</li>
          )}
          {redTeam.map((s) => {
            const sug =
              typeof s.suggestion_json === 'string'
                ? safeParse(s.suggestion_json)
                : s.suggestion_json;
            return (
              <li
                key={s.id}
              ...
              >
                <div className="flex items-center justify-between">
                  <span>
                    <span className="chip bg-slate-100 dark:bg-slate-700">{s.role}</span> ·{' '}
                    {s.status}
                  </span>
                  <span className="flex gap-2 text-xs">
                    {s.status !== 'accepted' && (
                      <button
                        className="text-emerald-600 hover:underline"
                        onClick={() => review.mutate({ sid: s.id, status: 'accepted' })}
                      >
                        Accepter
                      </button>
                    )}
                    {s.status !== 'rejected' && (
                      <button
                        className="text-amber-600 hover:underline"
                        onClick={() => review.mutate({ sid: s.id, status: 'rejected' })}
                      >
                        Rejeter
                      </button>
                    )}
                  ...
                  <div className="mt-1 text-slatewarn dark:text-slate-300">
                    ...
                    {Array.isArray(sug.missing_proofs) && sug.missing_proofs.length > 0 && (
                      <div className="mt-1 text-xs">
                        Preuves manquantes : {sug.missing_proofs.join(' · ')}
                      </div>
                    )}
                    {sug.could_change_recommendation && (
                      <div className="mt-1 text-xs text-amber-600">
                        Pourrait changer la recommandation : {sug.reason}
                      </div>
                    )}
                  </div>
                )}
...
function CompareStage({
  ...
  options: Row[];
  scores: Row[];
  audit: ReturnType<typeof normalizeAudit>;
  onDone: () => void;
}) {
  const scoreByOpt = useMemo(
    () => Object.fromEntries(scores.map((s) => [s.option_id, s])),
    [scores],
  );
  const hasProfile = Boolean(d.weight_profile_json);
  const setProfile = useMutation({
    mutationFn: () =>
      api.put(`/api/decisions/${id}/weight-profile`, {
        profile: 'standard',
  ...
  return (
    <section className="space-y-3">
      <div className="card">
        ...
        <p className="mt-1 text-sm text-slatewarn dark:text-slate-400">
          Le score ouvre une possibilité ; les vetos peuvent l’interdire.
        </p>
        {!hasProfile && (
          <button className="btn mt-2" onClick={() => setProfile.mutate()}>
            Activer le profil de poids standard
          </button>
        )}
      </div>
      {options.map((o) => (
        <ScoreEditor
          ...
          id={id}
          option={o}
          score={scoreByOpt[o.option_id]}
          onDone={onDone}
        />
      ))}
      <div className="card">
        <button className="btn" onClick={() => runAudit.mutate()} disabled={runAudit.isPending}>
          {fr.ws.runAudit}
        </button>
        {audit && (
          <div className="mt-3">
...
function ScoreEditor({
  ...
  const save = useMutation({
    mutationFn: () =>
      api.put<Row>(`/api/decisions/${id}/options/${option.option_id}/score`, { criteria }),
    onSuccess: onDone,
  });
  return (
    <div className="card">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold">
          {OPTION_TYPE_FR[option.type]} — {option.title}
        </h3>
        {score && (
          <span className="text-xs text-slatewarn dark:text-slate-400">
        ...
        {CRITERIA.map((c) => (
          <label key={c} className="text-xs">
            <span className="font-medium">
              {criterionLabels[c].label} ({criterionLabels[c].weight})
            </span>
            <input
              type="number"
              min={0}
              ...
              onChange={(e) =>
                setCriteria((p) => ({
                  ...p,
                  [c]: Math.max(0, Math.min(5, Number(e.target.value))),
                }))
              }
              className="field"
            />
      ...
      <button className="btn mt-3" onClick={() => save.mutate()} disabled={save.isPending}>
        Calculer le score
      </button>
    </div>
  );
...
function DecideStage({
  ...
  const set = (k: string, v: unknown) => setForm((f) => ({ ...f, [k]: v }));
  const setTT = (k: string, v: unknown) =>
    setForm((f) => ({ ...f, truth_test: { ...f.truth_test, [k]: v } }));
  const save = useMutation({
  ...
  const TT_FIELDS: [string, string][] = [
    ['critical_hypothesis', 'Hypothèse critique'],
    ['minimal_protocol', 'Protocole minimal'],
    ['max_duration', 'Durée max'],
    ['max_cost', 'Coût max'],
    ['success_signal', 'Signal de succès'],
    ['failure_signal', 'Signal d’échec'],
    ['decision_if_success', 'Décision si succès'],
    ['decision_if_failure', 'Décision si échec'],
  ];
  return (
    <section className="card">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        <Labelled label="Verdict">
          <select
            className="field"
            value={v}
            onChange={(e) => set('final_verdict', e.target.value)}
          >
            {Object.keys(verdictLabels).map((k) => (
              <option key={k} value={k}>
                {k} — {verdictLabels[k as keyof typeof verdictLabels].label}
              </option>
            ))}
          </select>
        </Labelled>
        <Labelled label="Option retenue">
          <select
            className="field"
            value={form.selected_option_id}
            onChange={(e) => set('selected_option_id', e.target.value)}
          >
            <option value="">—</option>
            {options.map((o) => (
              <option key={o.option_id} value={o.option_id}>
        ...
        <Labelled label="Seuil d’arrêt">
          <input
            className="field"
            value={form.stop_threshold}
            onChange={(e) => set('stop_threshold', e.target.value)}
          />
        </Labelled>
        <Labelled label="Date de revue">
          <input
            type="date"
            className="field"
        ...
        <label className="mt-6 flex items-center gap-2 text-sm">
          <input
            type="checkbox"
            checked={form.human_validation}
            onChange={(e) => set('human_validation', e.target.checked)}
          />
          Validation humaine
        </label>
      </div>
      ...
      {v === 'FAIRE' && (
        <Labelled label="Pourquoi FAIRE plutôt que TESTER ?" full>
          <input
            className="field"
            value={form.why_faire_not_tester}
            onChange={(e) => set('why_faire_not_tester', e.target.value)}
          />
        </Labelled>
      )}
      {v === 'TESTER' && (
        <fieldset className="mt-3 rounded-lg border border-slate-200 p-3 dark:border-slate-700">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {TT_FIELDS.map(([k, label]) => (
              <Labelled key={k} label={label}>
                <input
                  className="field"
                  value={form.truth_test[k] ?? ''}
                  onChange={(e) => setTT(k, e.target.value)}
                />
              </Labelled>
            ))}
          </div>
          <label className="mt-2 flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={Boolean(form.truth_test.can_kill_option)}
              onChange={(e) => setTT('can_kill_option', e.target.checked)}
            />
      ...
      {v === 'DIFFÉRER' && (
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          <Labelled label="Motif du report">
            <input
              className="field"
              value={form.defer_reason}
              onChange={(e) => set('defer_reason', e.target.value)}
            />
          </Labelled>
          <Labelled label="Signal de réouverture">
            <input
              className="field"
              value={form.reopening_signal}
              onChange={(e) => set('reopening_signal', e.target.value)}
            />
          </Labelled>
        </div>
      )}
      {v === 'ABANDONNER' && (
        <Labelled label="Disposition (archiver / transformer / surveiller / remplacer)" full>
          <input
            className="field"
            value={form.abandonment_disposition}
            onChange={(e) => set('abandonment_disposition', e.target.value)}
          />
        </Labelled>
      )}
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        {audit && (
          <span className={`chip ${auditClass[audit.audit_status]}`}>
...
function ExportsPanel({ id }: { id: string }) {
  ...
  const gen = useMutation({
    mutationFn: () =>
      api.post<{ files: { filename: string; content: string }[] }>(
        `/api/decisions/${id}/exports`,
        {},
      ),
  ...
  return (
    <div className="mt-4 border-t border-slate-200 pt-4 dark:border-slate-700">
      <div className="flex items-center gap-3">
        <button className="btn-ghost" onClick={() => gen.mutate()} disabled={gen.isPending}>
          Générer la note de décision (FR/EN)
        </button>
        {files && (
          <span className="text-xs text-slatewarn dark:text-slate-400">
      ...
      {fr_md && (
        <pre className="mt-3 max-h-96 overflow-auto rounded-lg bg-slate-50 p-3 text-xs dark:bg-slate-900">
          {fr_md.content}
        </pre>
      )}
    </div>
...
function AddForm({
  ...
  return (
    <form
      onSubmit={submit}
      className="mt-4 flex flex-wrap items-end gap-2 border-t border-slate-200 pt-3 dark:border-slate-700"
    >
      {fields.map((fl) => (
