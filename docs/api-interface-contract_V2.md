...
- **API version:** `0.2.0` — see the [Changelog](#7-changelog).
...
`{ key, title, description, columns[], rows[] }` (e.g. `criticality_score`, `control_concentration`,
`regime_assessment`, `event_pressure`, `flow_exposure`, `risk_state`, …). Taint-aware; 404 for a
...
- **API version `0.2.0`** — additive only vs 0.1.0; no client changes required.
